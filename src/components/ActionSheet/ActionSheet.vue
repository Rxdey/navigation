<template>
    <Teleport to="#home">
        <TransitionGroup name="right-inout" tag="div" class="action-wrap absolute bottom-18% right-0 z-9 flex-col items-end gap-48" @enter="onEnter" @leave="onLeave">
            <div class="action-item text-white h-90 relative right-0 flex-row items-center bg-black-30 w-50vw transition-50" v-for="(act, i) in actions" :class="{ 'action-animate': show }" :key="i" v-show="show" @click="onSelect(act, i)" :data-index="i" @transitionend="onClear" :style="{ scale: (i + 1) % 2 === 1 ? '1.2' : '1' }">
